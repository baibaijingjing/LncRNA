...
if($db eq "GRCh37" || $db eq "GRCh38"){
	open(COSMIC,"$Bin/bin/$db.cancer_gene_census.csv")||die $!;
	my $coshead=<COSMIC>;;
	my %cosmic=();
	while (<COSMIC>) {
		chomp;
...
open(OUT,">$outpath/$key.DEG.TF.txt")||die $!;
open(TF,"$Bin/bin/Animal_TFDB.db")||die $!;
my $tfhead=<TF>;
