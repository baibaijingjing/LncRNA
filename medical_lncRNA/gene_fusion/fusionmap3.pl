...
for (my $i=0;$i<@Bam ;$i++) {
	my $id = (split /\//,$Bam[$i])[-1];
	my $bam = "$Bam[$i]/accepted_hits.bam";
	push @Bam_file ,$bam ;
	push @ID, $id;
	print "$bam\n";
}
...
if ($type eq "Human.B38") {
	`ln -sf $Bin/ReferenceLibrary/Human.B38* $odir/fusionmap_result/ReferenceLibrary`;
}elsif ($type eq "Human.hg19") {
	`ln -sf $Bin/ReferenceLibrary/Human.hg19* $odir/fusionmap_result/ReferenceLibrary`;
}elsif ($type eq "Mouse.B38") {
	`ln -sf $Bin/ReferenceLibrary/Mouse.B38* $odir/fusionmap_result/ReferenceLibrary`;
}elsif ($type eq "Rat.B5.0") {
	`ln -sf $Bin/ReferenceLibrary/Rat.B5.0* $odir/fusionmap_result/ReferenceLibrary`;
}elsif ($type eq "Rat.B6.0") {
        `ln -sf $Bin/ReferenceLibrary/Rat.B6.0* $odir/fusionmap_result/ReferenceLibrary`;
}
...
for(my $i=0;$i<@Bam_file;$i++){
  ...
  print SH "perl $Bin/fusion_config/FusionMapconfigCreater.pl --bamfile $Bam_file[$i] --outputpath $odir/fusionmap_result/$ID[$i] --outputname $ID[$i] --datatype PE && ";
  if ($type eq "Human.B38") {
	...
	print SH "&&cat $odir/fusionmap_result/$ID[$i]/$ID[$i]\.PairedEndFusionReport.txt | awk '\$4>4 {print \$0}'|grep -v 'InBlackList|InFamilyList|InParalogueList|SameEnsemblGene' >$odir/fusionmap_result/$ID[$i]/$ID[$i]\.FusionReport_filter.txt&&sed  's/accepted_hits.bam.//g' -i $odir/fusionmap_result/$ID[$i]/$ID[$i]\.FusionReport_filter.txt\n";

}
close SH;
#die;
qsubOrDie("$odir/work_sh/fusionmap.sh",$queue,$cpu,$vf);
#system "cat $odir/fusionmap_result/$ID[$i]/$ID[$i]\.PairedEndFusionReport.txt | awk '\$4>4 {print \$0}' >$odir/fusionmap_result/$ID[$i]/$ID[$i]\.FusionReport_filter.txt";
 #`sed  's/accepted_hits.bam.//g' -i $odir/fusionmap_result/$ID[$i]/$ID[$i]\.FusionReport_filter.txt`;
...
for (my $i=0;$i<@ID ;$i++) {
 ...
 print SH "&& cp $odir/result/$ID[$i]/FusionReport.txt $odir/result/report/$ID[$i]\_FusionReport.xls\n";
...
print "[".&date_time_format(time())."]pfam annotation:\n";
print "[".&date_time_format(time())."]cat $odir/result/report/*_FusionReport.xls | less | grep -v FusionID | sort | uniq > $odir/pfam/fusion.gtf\n";
print "[".&date_time_format(time())."]perl $Bin/pfam/get_fusion_fa.pl -fa $genome -gtf  $odir/pfam/fusion.gtf -o $odir/pfam/fusion.fa\n\n";
my $gene=`wc -l $odir/pfam/fusion.gtf`;
my $gene_num=(split /\s/,$gene)[0];
#print $gene_num;
if ($gene_num<=30000){
`cat $odir/result/report/*_FusionReport.xls | less | grep -v "FusionID" | sort | uniq > $odir/pfam/fusion.gtf`;
`perl $Bin/pfam/get_fusion_fa.pl -fa $genome -gtf  $odir/pfam/fusion.gtf -o $odir/pfam/fusion.fa`;
print "[".&date_time_format(time())."]perl $Bin/pfam/cut_fa_file_to_dir.pl -mRNA $odir/pfam/fusion.fa -od $odir/pfam/mid -cut 200\n";
`perl $Bin/pfam/cut_fa_file_to_dir.pl -mRNA $odir/pfam/fusion.fa -od $odir/pfam/mid -cut 200`;
open (SH,">$shfile") || die "Can't creat $shfile, $!\n" ;
