...
print OUT "#GeneID\tChrom\tStrand\tASType\tStartSite\tEndSite\t\(StartSite\)'\t\(EndSite'\)\t\(ASSite\)\n";
open OUT1,">$ARGV[1]/$ARGV[2].IntronRetention.xls" || die;
print OUT1 "#GeneID\tChrom\tStrand\tASType\tStartSite\tEndSite\n";
open OUT2,">$ARGV[1]/$ARGV[2].skippedExon.xls" || die;
print OUT2 "#GeneID\tChrom\tStrand\tASType\tStartSite\tEndSite\n";
open OUT3,">$ARGV[1]/$ARGV[2].Alt3splice.xls" || die;
print OUT3 "#GeneID\tChrom\tStrand\tASType\tStartSite\tEndSite\tStartSite'\tEndSite'\tASSite\n";
open OUT4,">$ARGV[1]/$ARGV[2].Alt5splice.xls" || die;
print OUT4 "#GeneID\tChrom\tStrand\tASType\tStartSite\tEndSite\tStartSite'\tEndSite'\tASSite\n";
open OUT5,">$ARGV[1]/$ARGV[2].AltFirstExon.xls" || die;
print OUT5 "#GeneID\tChrom\tStrand\tASType\tStartSite\tEndSite\tStartSite'\tEndSite'\n";
open OUT6,">$ARGV[1]/$ARGV[2].AltLastExon.xls" || die;
print OUT6 "#GeneID\tChrom\tStrand\tASType\tStartSite\tEndSite\tStartSite'\tEndSite'\n";
...
#---------------------------- by Simon Young 2014-12-22 -----------------------------------
		my @exon = sort {$a<=>$b} @{$non_AS_array{$chr}{$gene}} if (defined $non_AS_array{$chr}{$gene} && @{$non_AS_array{$chr}{$gene}});
			...
			if ($AS_type eq "A5") {
				if ($graph_info{$chr}{$gene}{Strand} eq "+") {
					foreach my $start (sort keys %{$AS_info{A5}}) {
						...
						if ($#ends<1) {
							print TE "$gene\tAlt5' site\t$ends[0]\t$start\n";
						}
						elsif ($#ends==1) {
							my $alt_site = $ends[0]+1;
					...
					foreach my $end (sort{$a<=>$b} keys %{$AS_info{A5}}) {
						...
						if ($#starts<1) {
							print TE "$gene\tAlt5' site\t$starts[0]\t$end\n";
						}
						elsif ($#starts==1) {
							my $alt_site = $starts[1]-1;
