...
## get data
dat <- read.table(input, header=T, sep="\t", comment.char = "")
...
#bardat<-data.frame(sample=rep(colnames(dat),each=nrow(dat)),ASType=rep(rownames(dat),ncol(dat)),number=as.vector(dat))
#ASType<-rownames(dat)
