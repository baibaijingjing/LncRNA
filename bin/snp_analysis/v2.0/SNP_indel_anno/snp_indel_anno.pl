...
my %opts;
GetOptions(\%opts,"s=s","r=s","id=s","h" );

#&help()if(defined $opts{h});
if(!defined($opts{id}) || !defined($opts{s}) || !defined($opts{r}) || defined($opts{h}))
{
	print <<"	Usage End.";
	Description:
...
#my $snpEff = "/share/nas1/wangyj/bin/software/v3_6/snpEff/snpEff.jar" ;
...
$cmd="perl $Bin/SNPAnnotation_v1.2.pl -i $snpdir/SNP/final.indel.vcf -od $snpdir/SNP/indel_anno -o $species -r  $fa  -s $species -mode Indel -db $snpdir -n $snpdir/SNP/sample.list ";
print $cmd,"\n";
system $cmd;
...
$cmd="$Rscript $Bin/snp_indel_anno_plot.r $snpdir/stat/snp_anno/final_SNP.anno.stat $snpdir/stat/snp_anno ";
print $cmd,"\n";
system $cmd;

$cmd="$Rscript $Bin/snp_indel_anno_plot.r $snpdir/stat/indel_anno/final_Indel.anno.stat  $snpdir/stat/indel_anno ";
print $cmd,"\n";
system $cmd;
...
sub qsub()
{
	...
	$maxproc ||= 25 ;
	if (`hostname` =~ /cluster/){
		my $cmd = "perl /share/nas2/genome/bin/sh $config{qsub_sh} --maxproc $maxproc --reqsub $shfile --independent" ;
		&run_or_die($cmd);
	}
	else{
		my $cmd = "perl /share/nas2/genome/bin/sh $config{qsub_sh} --maxproc $maxproc --reqsub $shfile --independent" ;
...
#my $asamples = &get_sample_files(\@sample_files, $snpdir, $sample_list_file);
