...
plot_acgtn=function(infile,outfile=paste(infile,".png",sep=""),height=80,width=100){
  ...
  matplot(x=data[,1],y=data[,-1],type="l",lty=1,col=c("green3","red","blue","black","gray45"),axes=F,
          xlim=c(0,nrow(data)),main="Base Distribution",ylab="Percentage",xlab="",lwd=0.4)
  ...
  if(nrow(data)>200){
    ...
    axis(2,cex.axis=0.8,col.axis="gray25",lwd=0.5)
    ...
    pos[1]=1
    axis(1,labels=pos,at=pos,col="gray30",cex.axis=0.8,col.axis="gray25",lwd=0.5)
    axis(2,col="gray30",cex.axis=0.8,col.axis="gray25",lwd=0.5)
    mtext("Cycle Number",side=1,line=1.5,at=max(data[,1])/2,font=1,cex=0.6)
  }
  
  lgd=sub("\\(%\\)","",colnames(data)[-1])
  legend( "topright",lgd,col=c("green3","red","blue","black","gray45"),box.lty=1,bty="n",lwd=1,cex=0.5)
