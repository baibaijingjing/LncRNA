...
usage=function(){
  ...
  cat("
	...
	Options: 
		...
		outfile  	character 	the filename for output graph [optional, default: quality_bar.png]
		...
		col         character   the color of bar [optional, default: blue]
	example:
...
quality_bar=function(infile,outfile="quality_bar.png",height=80,width=120,col="#2171B5"){
  ...
  barplot(err,col=col,border=NA,axes =F,axisnames=F,space=0,
          ...
          main="Reads Average Error Rate",ylim=c(0,max(y.at)),xlim=c(0,max(out[,1])+1))
  ...
  if(length(err)>126){
    ...
    pos[1]=1
    at=out[,2]%in%pos
    axis(1,labels=rep(pos,times=2),at=out[at,1],col="gray30",cex.axis=0.8,col.axis="gray25",lwd=0.5)
    axis(2,at=y.at,labels=labels,col="gray30",cex.axis=0.8,col.axis="gray25",lwd=0.5)
    mtext("Read1",side=1,line=1.5,at=read/2,font=1,cex=0.6)
    mtext("Read2",side=1,line=1.5,at=read*1.5,font=1,cex=0.6)
  }else{
    pos=seq(0,length(err),25)
    pos[1]=1
    axis(1,labels=pos,at=out[pos,1],col="gray30",cex.axis=0.8,col.axis="gray25",lwd=0.5)
    axis(2,at=y.at,labels=labels,col="gray30",cex.axis=0.8,col.axis="gray25",lwd=0.5)
    mtext("Cycle Number",side=1,line=1.5,at=max(out[,1])/2,font=1,cex=0.6)
  }
