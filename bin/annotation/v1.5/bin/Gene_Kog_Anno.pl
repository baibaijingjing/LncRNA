...
&MKDIR("$odir/work_sh");
&MKDIR("$odir/work_sh/Kog_sh");
my $sh_dir="$odir/work_sh/Kog_sh";

my $blast_shell_file = "$odir/work_sh/Kog_sh/Kog.blast.sh";
my @subfiles;
@subfiles = glob("$id/*.fa");
...
foreach my $subfile (@subfiles) {
	...
	print OUT "$blastx -task blastx-fast -num_descriptions 100 -num_alignments 100 -evalue $Blast_e -db  $Database -query  $subfile -num_threads 2 -out  $odir/Kog_Dir/$name.Kog.blast &&\n";
}
close OUT;
...
`perl $Bin/bin/blast_format.pl -id $odir/Kog_Dir -od $Tab_dir -shdir $sh_dir -middir $odir/mid --kog  -queue $queues`;


`perl $Bin/bin/kog_parser.pl $Tab_dir/$Q_name.Kog.blast.tab.best `;
system "cp $Tab_dir/$Q_name.Kog.class $Result_dir/$Q_name.Kog_class.txt";
#`perl $Bin/bin/KogFunClassDrawer.pl -i $Tab_dir/$Q_name.Kog.class -o $Tab_dir/$Q_name.Kog.cluster.svg -png`;
...
{
	print <<"	Usage End.";
	Program:Blast && interproscan with KOG_DataBase Annotate New Gene
	Version: $version
