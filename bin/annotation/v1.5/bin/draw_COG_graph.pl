...
my $mid_file="$od/Cog_Mid_File_0";
for (my $i=0; ;$i++) {
	if (-f $mid_file) {
		my $now=$i+1;
		$mid_file="$od/Cog_Mid_File_".$now;
		next;
	}
	last;
}
...
open (OUT,">$od/$key.Cog.cluster.stat") || die "can't open file  $od/$key.Cog.cluster.stat\n ";
&OutHash(\%Data);
close(OUT);

`$config{Rscript} $Bin/cog_anno_plot.r $od/$key.Cog.cluster.stat $od/$key.Cog.cluster.png `;
