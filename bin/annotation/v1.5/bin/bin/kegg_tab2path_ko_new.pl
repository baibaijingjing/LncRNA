...
&USAGE unless ($ftab and $foutdir and $fkey and $anno);
...
########database file############
my $keggsmap="/share/nas3/xugl/Database/KEGG/png/latest";
$gene2pathway||="/lustre/xugl/";
$gene2KO||="/lustre/xugl";
my %result_path;
my %globe_path;
my %for_pie;
...
while (<TAB>) {
	...
	if (defined $genes_ko{$temp[0]}) {
		print OUTKO "$temp[0]\t$gene2K{$temp[0]}|$temp[-3]|$temp[-1]|$annotation{$temp[-1]}\n";
	}	
	if (defined $gene2path{$temp[4]}) {
		foreach my $pa (sort keys %{$gene2path{$temp[4]}}) {
			next unless (defined $gene2K{$temp[4]});
			if (defined $globemap{$pa}) {
				$globe_path{$pa}{$temp[0]} = $gene2K{$temp[4]};
			}else{
				$result_path{$pa}{$temp[0]} = $gene2K{$temp[4]};
			}
		}
	}
}
