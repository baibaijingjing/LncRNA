...
my ($cog_best,$nr_best,$nt_best,$trembl_best,$swissprot_best,$kegg_best,$go,$kog_best);
my ($cog,$nr,$nt,$trembl,$swissprot,$kegg,$kobas,$kog);
my $anno_file=$file_db{kegg_anno_file};

my @annos = split /,/,$Anno_dir;
foreach my $anno_dir (@annos) {
	$anno_dir = &ABSOLUTE_DIR($anno_dir);
	my @best_tabs = glob "$anno_dir/02.gene-annotation/*.blast.tab.best" ;
	#push @best_tabs,"$anno_dir/02.gene-annotation/kobas.annotation" ;
	foreach my $file (@best_tabs) {
		$file =~ /(.*)\.best/;
		my $tab = $1;
		if ($file =~ /\.cog\./i) {
			$cog_best .= "$file ";
	...
	my @go_annot = glob "$anno_dir/Result/*.annot";
	if ($#go_annot >= 0) {
		$go .= "$go_annot[0] ";
	}
}
...
####### Anno Integrate
if((defined $cog ||defined $kog) && (defined $nr || defined $nt || defined $swissprot || defined $trembl || defined $kegg)){
