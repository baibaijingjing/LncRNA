...
if (defined $eggNOG) {
	...
	print OUT "cat $eggNOG_class >$odir/02.gene-annotation/$out_index.eggNOG_class \n";
	close OUT;

	system "sh $sh_dir/eggNOG_cat.sh";
	system "sed -i \'2,\$s\/^Query.*\/\/g\' $odir/02.gene-annotation/$out_index.eggNOG.blast.tab";
	system "sed -i \'2,\$s\/^#.*\/\/g\' $odir/02.gene-annotation/$out_index.eggNOG_class";
	system  "sed -i \'\/^\$\/d\'   $odir/02.gene-annotation/$out_index.eggNOG.blast.tab ";
        system "sed -i \'\/^\$\/d\'   $odir/02.gene-annotation/$out_index.eggNOG_class ";

	open OUT,">$sh_dir/eggNOG_tab2class.sh" || die $!;

	print OUT "perl $Bin/bin/eggNOGClassDrawer.pl -i $odir/02.gene-annotation/$out_index.eggNOG_class -o $odir/Result/$out_index.eggNOG.cluster \n";
	close OUT;

	system "sh $sh_dir/eggNOG_tab2class.sh";

	system "cp $odir/02.gene-annotation/$out_index.eggNOG_class $odir/Result/$out_index.eggNOG_class.txt";
