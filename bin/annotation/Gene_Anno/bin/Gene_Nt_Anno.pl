...
&MKDIR("$odir/work_sh");
&MKDIR("$odir/work_sh/Nt_sh");
my $sh_dir="$odir/work_sh/Nt_sh";

my $blast_shell_file = "$odir/work_sh/Nt_sh/Nt.blast.sh";
my @subfiles;
@subfiles = glob("$id/*.fa");
...
foreach my $subfile (@subfiles) {
	...
	print OUT "$blastn -task blastn-short  -num_descriptions 100 -num_alignments 100 -evalue $Blast_e -db  $Database -query  $subfile -num_threads 6 -out  $odir/Nt_Dir/$name.nt.blast &&\n";
}
close OUT;
...
`perl $Bin/bin/blast_format.pl -id $odir/Nt_Dir -eval $Blast_e -od $Tab_dir -shdir $sh_dir  -middir $odir/mid --nt -queue $queues`;


####################
open (NT,"$Tab_dir/$Q_name.nt.blast.tab.best")or die "cant open file $Tab_dir/$Q_name.nt.blast.tab.best";
open (OUT,">$Tab_dir/$Q_name.nt.blast.tab.best.anno")or die "cant open file $Tab_dir/$Q_name.nt.blast.tab.best.anno";
...
`cp $Tab_dir/$Q_name.nt.blast.tab.best.anno $Result_dir/$Q_name.nt.anno.txt`;
