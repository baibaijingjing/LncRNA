...
=pod
	if (@anno==7){
		$Anno_Gene{$anno[0]}{eggNOG}="$anno[-3]\t$anno[-1]";
	}elsif(@anno==6){
		$Anno_Gene{$anno[0]}{eggNOG}="$anno[4]\t--";
	}
=cut
	$Anno_Gene{$anno[0]}{eggNOG}="$anno[-3]\t$anno[-1]";	
...
foreach (sort keys %Anno_Stat) {
	next if (/All/) ;
		$Anno_Stat{$_}{Anno}=0 unless (exists $Anno_Stat{$_}{Anno});
		$Anno_Stat{$_}{300}=0  unless (exists $Anno_Stat{$_}{300});
		$Anno_Stat{$_}{1000}=0 unless (exists $Anno_Stat{$_}{1000});
	if ($_!~/zz/) {
