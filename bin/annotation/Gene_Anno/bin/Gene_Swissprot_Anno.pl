...
&MKDIR("$odir/Swissprot_Dir");
&MKDIR("$odir/work_sh");
&MKDIR("$odir/02.gene-annotation");
my $Tab_dir="$odir/02.gene-annotation";
&MKDIR("$odir/work_sh");
&MKDIR("$odir/work_sh/Swissprot_sh");
my $sh_dir="$odir/work_sh/Swissprot_sh";

my $blast_shell_file = "$odir/work_sh/Swissprot_sh/Swissprot.blast.sh";
my @subfiles;
@subfiles = glob("$id/*.fa");
...
foreach my $subfile (@subfiles) {
	...
	print OUT "$blastx  -task blastx-fast -max_target_seqs 100 -evalue $Blast_e -db  $Database -query  $subfile -outfmt 5 -num_threads 2 -out  $odir/Swissprot_Dir/$name.Swissprot.blast.xml &&\n";
}
close OUT;
...
`perl $Bin/bin/blast_format.pl -id $odir/Swissprot_Dir -od $Tab_dir -shdir $sh_dir  -middir $odir/mid --swissprot -queue $queues`;


open (SWISS,"$Tab_dir/$Q_name.Swissprot.blast.tab.best")or die "cant open file $Tab_dir/$Q_name.Swissprot.blast.tab.best";
open (OUT,">$Tab_dir/$Q_name.Swissprot.blast.tab.best.anno")or die "cant open file $Tab_dir/$Q_name.Swissprot.blast.tab.best.anno";
...
`cp $Tab_dir/$Q_name.Swissprot.blast.tab.best.anno $Result_dir/$Q_name.Swissprot.anno.txt`;
...
{
	print <<"	Usage End.";
	Program:Blast && interproscan with Swissprot_DataBase Annotate New Gene
	Version: $version
