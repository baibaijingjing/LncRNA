...
my %CFG;
&LOAD_PARA($cfg,\%CFG);
if (!exists $CFG{'Kog'}){$CFG{'Kog'}=$config{'Kog'};}
if (!exists $CFG{'Pfam'}){$CFG{'Pfam'}=$config{'Pfam'};}
if (!exists $CFG{'eggNOG'}){$CFG{'eggNOG'}=$config{'eggNOG'};}
$Query=$CFG{mRNA};
...
#######################################################################################
&show_log2("step_4: New gene functional annotation start.");
# ------------------------------------------------------------------
# preparation
# ------------------------------------------------------------------
...
if ($step == 2) {
    ...
    if (defined $eggNOG ||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_eggNOG_Anno.pl  -id $mid_dir -Database $CFG{eggNOG} -od $odir -cpu $Blast_cpu -queue  $queue\n";
		close SH;
		push @work_sh,$SH;
    }
    ...
    if (defined $Pfam||defined $all) {
		...
		open (SH,">$SH") or die $!;
        print SH "perl $Bin/bin/Gene_Pfam_Anno.pl --idir $mid_dir --pfam $CFG{Pfam} --odir $odir --cpu $hmmscan_cpu -queue  $queue\n";
		close SH;
		push @work_sh,$SH;
    }
...
if ($step == 4) {
    ...
    #    system "perl $Bin/bin/draw_COG_graph.pl -i $Result_dir/All_Database_annotation.xls -k $Q_name -od $Result_dir " if ($Cog||$all);
    ...
    #    system "perl $Bin/bin/draw_KOG_graph.pl -i $Result_dir/All_Database_annotation.xls -k $Q_name -od $Result_dir " if ($Kog||$all);
    #}
