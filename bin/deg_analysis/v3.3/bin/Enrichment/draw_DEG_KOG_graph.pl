...
mkdir "$od/Kog_Anno" unless -d "$od/Kog_Anno";
...
open (OUT,">$od/Kog_Anno/$key.Kog.classfy.stat") || die "can't open file  $od/Kog_Anno/$key.Kog.classfy.stat\n ";
&OutHash(\%Data);
close(OUT);

#`/share/nas2/genome/biosoft/R/3.1.1/bin/Rscript $Bin/kog_anno_plot.r $od/Kog_Anno/$key.Kog.classfy.stat $od/Kog_Anno/$key.Kog.classfy.png `;
`$config{Rscript} $Bin/kog_anno_plot.r $od/Kog_Anno/$key.Kog.classfy.stat $od/Kog_Anno/$key.Kog.classfy.png `;
