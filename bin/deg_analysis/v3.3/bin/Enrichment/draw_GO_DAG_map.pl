...
my $Rscript = $config{Rscript} ||  "/share/nas2/genome/biosoft/R/3.1.1/bin/Rscript"; #2016-01-04 ~ linhj
...
#`/share/nas2/genome/biosoft/R/3.0.1/bin/Rscript $od/topGO.R`;
system "$Rscript $od/topGO.R";

chdir $od;
system "$config{svg2xxx} $key.topGO_BP.svg";
system "$config{svg2xxx} $key.topGO_MF.svg";
system "$config{svg2xxx} $key.topGO_CC.svg";
#`rm $od/topGO.R`;
