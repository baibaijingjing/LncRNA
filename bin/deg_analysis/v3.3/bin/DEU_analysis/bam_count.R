...
bam_count=function(
                   infile,gtf,od,type="sam",
                   queue="general.q",cpu=NULL,vf="5G"
                   ){
  library(DEXSeq)
  ...
  ID=regmatches(samfile,regexpr("[LT][0-9]{1,2}", samfile))
  count.fl=paste(od,ID,".count",sep="")
  ct.cd=paste(python,count.py,gff,samfile, count.fl,"-p yes -s no -r pos -f",type)
  shfile=paste(od,"count.sh",sep="")
  cat(ct.cd,file=shfile,sep="\n")
  
  #--------------make qsub command and run----------------------------------------------------------------
  if(is.null(cpu)){cpu=length(ID)}
  shcm=paste("sh /share/nas3/xugl/Programs/Submit/qsub_sge_current.sh ",shfile,
             " --queue ",queue ," --maxproc ",cpu," --resource vf=",vf ,
             " --reqsub --independent",sep="")
  system(shcm)
  #-------------------------------------------------------------------------------------------------------
