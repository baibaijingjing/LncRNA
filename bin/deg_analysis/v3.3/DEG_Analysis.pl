...
&show_log2("step_9: Difference expression genes analysis of mRNA start.");
###########################################################
...
#	print SH2 "perl $Bin/bin/draw_cor_plot/v1.0/draw_cor_plot.pl -treated $treated -control $control -all $dir/$group_name.final.xls -o $dir/$group_name.cor.png \n";
	...
	if (defined $enrich) {
		...
		print SH3 "perl $Bin/bin/Enrichment/draw_DEG_eggNOG_graph.pl -i $enrich -deg $dir/$group_name.DEG_final.xls -k $group_name -od $dir/Anno_enrichment\n";
...
#########draw anno_cluster heatmap 
print "perl $Bin/bin/draw_anno_cluster/draw_anno_cluster.pl -i $enrich -deg $od/All_DEG/All.DEG_final.xls -od $od/Anno_Cluster/ \n";
system "perl $Bin/bin/draw_anno_cluster/draw_anno_cluster.pl -i $enrich -deg $od/All_DEG/All.DEG_final.xls -od $od/Anno_Cluster/ ";
############所有差异基因聚类作图
