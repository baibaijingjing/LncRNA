...
## draw venn picture
	...
	my $margin='margin=0.15';
	my $height='height=1500';
	my $width='width=1500';
	my $resolution='resolution=300';
	my $units="units='px'";
	my $lwd='lwd=1';
	my $fill="fill=c($colors)";
	my $cex=($sample_num==5)?'cex=1.8':'cex=1.0';
	my $cat_cex='cat.cex=1.0';
	my $scaled='scaled=0';
	my $cat_pos=($sample_num==2)?'cat.pos=c(0,6)':'';
	my $out_pic="filename=\"All_DEG_veen.png\"";
	my $cat_dist=($sample_num==2)?'cat.dist=c(1.32,1.77)':'';	
	$cat_dist=($sample_num==3)?'cat.dist=c(0.12,0.14,0.07)':'';#0.12,0.12,0.07
	my $venn_command="venn.diagram($list,$margin,$height,$width,$resolution,$units,$lwd,$fill,$cex,$cat_cex,$cat_pos,$scaled,$category_name,$out_pic,$cat_dist,imagetype=\"png\")"."\n";
	open OUT,">","venn.R";
