...
print_usage <- function(spec=NULL){
  ...
  cat("
      Usage example: 
      Rscript id_conversion.r --infile in.xls --outfile out.xls  --id_name ensembl_gene_id --id.col 1 \\
      --database ensembl --dataset rnorvegicus_gene_ensembl \\1
      Options: 
...
# reading data
#-----------------------------------------------------------------
data <- read.table(opt$infile, skip=opt$skip,header = T,sep = "\t",fill=TRUE,comment.char="")
# check dim
data.dim <- dim(data)
if ( is.null(data.dim) ){
...
# output
#-----------------------------------------------------------------
write.table(newdata,file = paste(opt$outfile,".xls",sep=""),row.names = F,col.names = T,quote = F,sep="\t")
