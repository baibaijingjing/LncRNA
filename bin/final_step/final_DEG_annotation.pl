...
chomp $link;
print WRITE "$link\t";
my @links = split /\t/, <READ_2>;
chomp @links;
print WRITE
  "$links[-6]\t$links[-5]\t$links[-4]\t$links[-3]\t$links[-2]\t$links[-1]\n";
...
while (<READ_1>) {
    ...
    while (<READ_3>) {
        ...
        if (/$array[0]/) {
            push @array,
              ( $err[-6], $err[-5], $err[-4], $err[-3], $err[-2], $err[-1] );
            $final = join( "\t", @array );
            chomp $final;
...
#-----------------------------------------------------------
 ...
 Data:2015-09-08
Function:assembly sequence,add annotation (six kinds)
   USAGE:
...
__USAGE__
    print $usage;    #打印帮助信息
    exit
      ; #当调用该子程序时表示程序无法正常执行，使用exit函数强行退出该程序
}
