...
our %opts;
GetOptions(\%opts,"cfg=s","od=s","h","tophat=s","gtfdir=s");


if(!defined($opts{cfg}) || !defined($opts{od}) || !defined($opts{tophat}) || defined($opts{h}) || !defined($opts{gtfdir})){
	&help();
	exit;
}
...
&show_log2("step_8: Differential exon usage analysis start.");
###########################################################
...
##############################
&show_log2("step_8: Differential exon usage analysis finished.");
#close LOG;
##############################
...
sub control_DE_analysis{
	...
	if( exists $$control{Sep} ) {
		...
		#if ($deu=="ok"){continue;}else {die;}
		my $tophat=$opts{tophat};
		my $strand;
		if ($$control{Lib_type} eq "fr-firststrand"){
			$strand="yes";
		}
		if ($$control{Lib_type} eq "fr-secondstrand"){
			$strand="reverse";
		}else{
			$strand="no";
		}
		...
		for(my $i=0; $i<$len; $i++) {
			compare_with_replicates($$sep_groups[$i], $outdir, $gtf, $tophat,$strand);
		}
	}
	close SH;
...
sub compare_with_replicates {
	# get para
	my ($sample_id, $outdir,$gtf,$tophat,$strand) = @_;
################create the path.txt used for DEU_analysis
	...
	for (my $i=0;$i<=$#groups-1;$i++) {
		for (my $j=$i+1;$j<=$#groups;$j++) {
			...
			my $cmd="$Rscript $Bin/bin/DEU_DEXSeq.R  infile=$outdir/htseq_count/path.txt group=$vs_out od=$outdir/$vs_out FDR=$CONTROL{FDR} ";
                        print SH "$cmd\n";
		}
	}
}
...
-tophat		  dir for DEU analysis                            must be given;
