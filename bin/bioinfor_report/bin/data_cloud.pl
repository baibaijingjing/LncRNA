...
while (<IN>) {
	chomp;
	next if (/^#/);
	my ($BMK_ID,$readsum,$basesum,$GC,$N,$Q30) = split /\t/,$_;
...
########### Get $new_gene_num
chomp (my $new_gene_num = `grep ">" $dir_abs/NewGene/*.newGene.longest_transcript.fa |wc -l`)	;
...
chomp (my $deg_num = `less $dir_abs/DEG_Analysis/All_DEG/All.DEG_final.xls |wc -l`);
$deg_num = $deg_num-1;

chomp (my $diff_lnc = `less $dir_abs/Lnc_Diff_Analysis/All_DEG/All.DEG_final.xls |wc -l`);
#$diff_lnc = $diff_lnc-1;
if ($diff_lnc ==0) {
    $diff_lnc =0;
}else{
...
chomp (my $lnc_num = `less $dir_abs/LncRNA/LncRNA.id |wc -l`);


my $optimized_gene_path = glob("$dir_abs/Gene_Structure_Optimize/*.xls");
...
sub get_table {
	chomp (my $id = shift);
	%table_info=(
		"样品测序数据评估统计表" => "$dir_abs/rawdata/AllSample_GC_Q.stat",
		"Clean Data与参考基因组比对结果统计表" => "$dir_abs/geneExpression/*.mappedStat.xls",
		"新基因功能注释结果统计" => "$dir_abs/NewGene/NewGene_Anno/Function_Annotation.stat.xls",
	);
