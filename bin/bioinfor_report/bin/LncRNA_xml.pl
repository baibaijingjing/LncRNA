...
sub table_write {
	...
	if (1<$table_num) {
		...
		if ($desc=~/SNP位点信息/){
			...
			my $txt_path=shift;
			if($txt_line>30){
				`head -n 11 $txt >$dir/template/Cufflinks.result`;
			...
			my $txt_path=shift;
			if($txt_line>30){
				`head -n 11 $txt >$dir/template/Scripture.result`;
			...
			$content ="<$tit_class name=$tit type=$type desc=$tit \/> \n";
			$content .="<p type=$type desc=$ann \/> \n";
			my $name1="Scripture拼接结果示意表"; $name1=&T($name1);
			my $path1="$dir_template/Scripture.gtf"; $path1=&T($path1);
			$content .="<table name=$name1 type=$type path=$path1 \/> \n";
			$content .="<p type=$type desc=$ann1 \/> \n";
			$content .="<file_list name=$desc type=$type> \n";
			foreach my $table(@tables){
				my $table_name=&T(basename($table));
...
sub get_picture {
	...
	%pic_info = (
		...
		"mRNA与lncRNA共表达网络分析示意图"=>"$dir_template/WGCNA.png",
