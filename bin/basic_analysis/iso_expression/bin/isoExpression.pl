...
# GetOptions
# ------------------------------------------------------------------
my ($cuffnorm_dir,$final_track_dir,$outdir);
...
&USAGE unless ($cuffnorm_dir and $final_track_dir and $outdir);
...
open (TRACK,"$final_track_dir") or die $!;
...
while (<TRACK>) {
	...
	my $len=$col[4]-$col[3]+1;
	if (exists $track{$trans_id}{'start'}){
		$track{$trans_id}{'start'}=($track{$trans_id}{'start'} < $col[3]) ? $track{$trans_id}{'start'} :$col[3];
	}else{
		 $track{$trans_id}{'start'}=$col[3];
	}
	if (exists  $track{$trans_id}{'end'}){
		$track{$trans_id}{'end'}=($track{$trans_id}{'end'} > $col[4])?$track{$trans_id}{'end'}:$col[4];
	}else{
		$track{$trans_id}{'end'}=$col[4];
	}
	
	$track{$trans_id}{'locus'}=$col[0];
...
#	my $basic_inf=join "\t",($track{$i}{'iso_id'},$track{$i}{'length'},$track{$i}{'stand'},$track{$i}{'gene_id'},$track{$i}{'locus'});
	my $locu="$track{$i}{'locus'}".":".$track{$i}{'start'}."-".$track{$i}{'end'};
#	my $basic_inf=join "\t",($track{$i}{'trans_id'},$track{$i}{'length'},$track{$i}{'stand'},$track{$i}{'locus'});
	my $basic_inf=join "\t",($track{$i}{'trans_id'},$track{$i}{'length'},$track{$i}{'stand'},$locu);
	my $print=$basic_inf;
