...
foreach my $chr (sort keys %track2chr) {
	foreach my $gene_track (sort keys %{$track2chr{$chr}}) {
		print OUT1 "$chr\tCufflinks\tgene\t$gene_boundary{$gene_track}{Start}\t$gene_boundary{$gene_track}{End}\t.\t$gene_boundary{$gene_track}{Strand}\t.\tID=$gene_track2name{$gene_track}\n";
		foreach my $iso_track (sort keys %{$gene_iso{$gene_track}}) {
			print OUT1 "$chr\tCufflinks\tmRNA\t$iso_info{$gene_track}{$iso_track}{Start}\t$iso_info{$gene_track}{$iso_track}{End}\t.\t$iso_info{$gene_track}{$iso_track}{Strand}\t.\tID=$iso_track2name{$iso_track}".";Parent=$gene_track2name{$gene_track}\n";
			my @exons = sort {$a<=>$b} @{$iso_info{$gene_track}{$iso_track}{Exonarray}};
			my $length_0;
			for (my $i=0 ; $i < $#exons ; $i+=2) {
				print OUT1 "$chr\tCufflinks\tCDS\t$exons[$i]\t$exons[$i+1]\t.\t$iso_info{$gene_track}{$iso_track}{Strand}\t.\tParent=$iso_track2name{$iso_track}\n";
...
foreach my $gene_name (sort keys %gene_model_divided) {
	foreach my $gene_track (sort keys %{$gene_model_divided{$gene_name}}) {
		print OUT2 "$chr2track{$gene_track}\tCufflinks\tgene\t$gene_boundary{$gene_track}{Start}\t$gene_boundary{$gene_track}{End}\t.\t$gene_boundary{$gene_track}{Strand}\t.\tID=$gene_track2name{$gene_track}".";RefID=$gene_name\n";
		foreach my $iso_track (sort keys %{$gene_iso{$gene_track}}) {
			print OUT2 "$chr2track{$gene_track}\tCufflinks\tmRNA\t$iso_info{$gene_track}{$iso_track}{Start}\t$iso_info{$gene_track}{$iso_track}{End}\t.\t$iso_info{$gene_track}{$iso_track}{Strand}\t.\tID=$iso_track2name{$iso_track}".";Parent=$gene_track2name{$gene_track}\n";
			my @exons = sort {$a<=>$b} @{$iso_info{$gene_track}{$iso_track}{Exonarray}};
			for (my $i=0 ; $i < $#exons ; $i+=2) {
				print OUT2 "$chr2track{$gene_track}\tCufflinks\tCDS\t$exons[$i]\t$exons[$i+1]\t.\t$iso_info{$gene_track}{$iso_track}{Strand}\t.\tParent=$iso_track2name{$iso_track}\n";
			}
		}
	}
}
...
foreach my $gene_track (sort keys %gene_model_integration) {
	print OUT3 "$chr2track{$gene_track}\tCufflinks\tgene\t$gene_boundary{$gene_track}{Start}\t$gene_boundary{$gene_track}{End}\t.\t$gene_boundary{$gene_track}{Strand}\t.\tID=$gene_track2name{$gene_track}".";RefID=$gene_model_integration{$gene_track}\n";
	foreach my $iso_track (sort keys %{$gene_iso{$gene_track}}) {
		print OUT3 "$chr2track{$gene_track}\tCufflinks\tmRNA\t$iso_info{$gene_track}{$iso_track}{Start}\t$iso_info{$gene_track}{$iso_track}{End}\t.\t$iso_info{$gene_track}{$iso_track}{Strand}\t.\tID=$iso_track2name{$iso_track}".";Parent=$gene_track2name{$gene_track}\n";
		my @exons = sort {$a<=>$b} @{$iso_info{$gene_track}{$iso_track}{Exonarray}};
		for (my $i=0 ; $i < $#exons ; $i+=2) {
			print OUT3 "$chr2track{$gene_track}\tCufflinks\tCDS\t$exons[$i]\t$exons[$i+1]\t.\t$iso_info{$gene_track}{$iso_track}{Strand}\t.\tParent=$iso_track2name{$iso_track}\n";
		}
	}
}
close OUT3;
