...
spec = matrix(c(
  ...
  'clustmethod' ,   'l', 1, "character",#'color' , 'r', 1, "character", #样本颜色
  'samplenum',      'n',1,"integer",
  'sampletype' ,    't', 1, "character",#样本类型
...
###get gene fpkm
a = list.files(opt$indir)           #list.files命令将当前文件夹下所有文件名输入
expression_file = a[grep(".geneExpression.xls$",a)]
dirfile = paste(opt$indir, expression_file,sep="/")     #用paste命令构建路径变量dir
n = length(dirfile)                #读取dir长度，也就是文件夹下的文件个数
merge.data = read.table(file = dirfile[1])   #读入第一个文件内容（可以不用先读一个，但是为了简单，省去定义data.frame的时间，我选择先读入一个文件。
merge.data = merge.data[,c(1,3)]
for (i in 2:n){
  new.data = read.table(file = dirfile[i])
  new.data = new.data[,3]
  merge.data = cbind(merge.data,new.data)
}
colnames(merge.data)[1:(n+1)]=c("Gene_ID",as.vector(as.data.frame(strsplit(expression_file,".geneExpression."))[1,]))
rownames(merge.data)=merge.data[,1]
write.table(merge.data, file=paste0(opt$outdir,"FPKM_gene.xls"),row.names=F, col.names=T, quote=FALSE,sep="\t")


###get lncRNA fpkm
b=list.files(paste(opt$indir,"../LncExpression/",sep="/"))
lncRNA_expression_file = b[grep(".isoExpression.xls$",b)]
lncRNA_dirfile = paste(opt$indir,"../LncExpression",lncRNA_expression_file,sep="/")
lncRNA_merge.data = read.table(file = lncRNA_dirfile[1])
lncRNA_merge.data = lncRNA_merge.data[,c(1,5)]
for (i in 2:n){
  new.data = read.table(file = lncRNA_dirfile[i])
  new.data = new.data[,5]
  lncRNA_merge.data = cbind(lncRNA_merge.data,new.data)
}
colnames(lncRNA_merge.data)[1:(n+1)]=c("Gene_ID",as.vector(as.data.frame(strsplit(lncRNA_expression_file,".isoExpression."))[1,]))
rownames(lncRNA_merge.data)=lncRNA_merge.data[,1]

lncRNA_id_file=paste(opt$indir,"../Tophat_Cufflinks/Lnc_filter/lnc_filter_id.list",sep="/")
lncRNA_id=read.table(file = lncRNA_id_file,header=F,sep="\t")
lncRNA_fpkm=lncRNA_merge.data[pmatch(as.vector(lncRNA_id[,1]),as.vector(lncRNA_merge.data[,1])),]
write.table(lncRNA_fpkm, file=paste0(opt$outdir,"FPKM_lncRNA.xls"),row.names=F, col.names=T, quote=FALSE,sep="\t")

merge.data.final=rbind(merge.data,lncRNA_fpkm)
colnames(merge.data.final)[1:(n+1)]=c("Gene_ID",as.vector(as.data.frame(strsplit(expression_file,".geneExpression."))[1,]))
...
fpkm=aa[aa[,n+1] > opt$meanFPKM ,1:n]  #筛选之后的结果取FPKM均值大于0.1的较合适
...
plot(as.dendrogram(hr),
     ...
     cex=0.5)
dev.off()
png(filename = paste0(opt$outdir,"Fig_1_hclust.png"),width= 6000,height = 4800,res=800)      ###################################增加png的输出############
plot(as.dendrogram(hr),
     edgePar=list(col=1, lwd=2),
     horiz=F,
     main=paste("Sample Clustering by",opt$cormethod),
...
dev.off()
png(filename = paste0(opt$outdir,"Fig_2_PCA_result_2D.png"),width= 6000,height = 4800,res=800)      ###################################增加png的输出############
#plot(pca$x[,c(1,2)],pch=16,col=rep(rainbow(length(opt$sampletype)),each=opt$samplenum/2),cex=1.5,main = "PCA map")
plot(pca$x[,c(1,2)],pch=16,col=rep(rainbow(opt$samplenum),each=1),cex=1.5,main = "PCA map")
text(pca$x[,c(1,2)],row.names(pca$x),col="black",pos=3)
#legend("topleft",legend=opt$sampletype,
...
if(opt$samplenum >= 3){
	...
	dev.off()
	png(filename = paste0(opt$outdir,"Fig_3_PCA_result_3D.png"),width= 6000,height = 4800,res=800)      ###################################增加png的输出############
	#scatterplot3d(pca$x[,1:3], highlight.3d=F, col.axis="black",color = rep(rainbow(length(opt$sampletype)),each=opt$samplenum/2),cex.symbols=1.5,cex.lab=1.5,cex.axis=1.4,
	#	      col.grid="lightblue", main="PCA map", pch=16)
	#legend("topright",legend = opt$sampletype ,
	#       pch=16,cex=1.2,col=rainbow(length(opt$sampletype)), ncol = 1) #"bottomright"
...
dev.off()
png(filename = paste0(opt$outdir,"Fig_4_sample_correlation_pheatmap.png"),width= 6000,height = 4800,res=800)      ###################################增加png的输出############
pheatmap(c,color=colorRampPalette(c("green","black","red"))(100),border_color = NA,
         main = "Sample Correlation")
dev.off()
...
if(opt$samplenum >= 5){
	...
	#par(mar = c(1,12,2,1))
	#sizeGrWindow(12, 9)
	pdf(file = paste0(opt$outdir,"Fig_5_Sample dendrogram and trait heatmap.pdf"))      ########################增加pdf输出
	...
	dev.off()
	png(filename = paste0(opt$outdir,"Fig_5_Sample dendrogram and trait heatmap.png"),width= 6000,height = 4800,res=800)  #修改大小，像素使显示更加清晰
	plotDendroAndColors(sampleTree2, traitColors,
			    groupLabels = row.names(datTraits),
			    marAll = c(1, 8, 3, 1),
			    cex.rowText = 0.8,
...
#---------------------------------------------------------------------#
	...
	png(filename = paste0(opt$outdir,"Fig_6_Scale Free Topology and mean.png"),width = 6000, height = 4800,res=800);
	par(mfrow = c(1,2));
	par(mar=c(3,5,3,3))
	cex1 = 0.9;
	...
	dev.off()
	pdf(file = paste0(opt$outdir,"Fig_6_Scale Free Topology and mean.pdf"));         #############################增加一个PDF输出###################                        
	par(mfrow = c(1,2));
	par(mar=c(3,5,3,3))
	cex1 = 0.9;
	# Scale-free topology fit index as a function of the soft-thresholding power
...
#---------------------------------------------------------------------
	...
	dev.off()
	pdf(file = paste0(opt$outdir,"Fig_7_1_Gene clustering on TOM-based dissimilarity.pdf"))         #################增加一个pdf输出##########
	plot(geneTree, xlab="", sub="", main = "Gene clustering on TOM-based dissimilarity",
	     labels = FALSE, hang = 0.04,cex=2)
	dev.off()
#---------------------------------------------------------------------
...
#---------------------------------------------------------------------
	...
	dev.off()
	pdf(file = paste0(opt$outdir,"Fig_7_2_Gene dendrogram and module colors.pdf"))        ###############增加一个PDF输出################
	plotDendroAndColors(geneTree, dynamicColors, "Dynamic Tree Cut",
			    dendroLabels = FALSE, hang = 0.03,
			    addGuide = TRUE, guideHang = 0.05,
			    main = "Gene dendrogram and module colors")
...
#---------------------------------------------------------------------
	...
	for (module in intModules)
	{
	  ...
	  axis(1,at=seq(1:15),labels=F,tick=T,ylab="eigengene expression",
	       xlab="array sample")
	  par(mar=c(0, 0, 2.5, 0))
	 pie(c((length(names(datExpr[,moduleColors==module]))-length(grep("newGene",names(datExpr[,moduleColors==module])))-length(grep("TCONS",names(datExpr[,moduleColors==module])))),
	    length(grep("newGene",names(datExpr[,moduleColors==module]))),length(grep("TCONS",names(datExpr[,moduleColors==module])))),invert=T,
	  labels=paste(c("Known_gene","new_gene","new_lncRNA"),
		       c((length(names(datExpr[,moduleColors==module]))-length(grep("newGene",names(datExpr[,moduleColors==module])))-length(grep("TCONS",names(datExpr[,moduleColors==module])))),
			 length(grep("newGene",names(datExpr[,moduleColors==module]))),length(grep("TCONS",names(datExpr[,moduleColors==module])))),
		       sep="\n"),
