...
sub get_id {
	...
	while($line=<IN>){
		...
		if(exists $id{$str[$idcol]}){
			next;#$bmk->logAndDie(-info=>"repeat id ($str[$idcol])");
		}else{
			$id{$str[$idcol]} = 1;
		}
	}
...
Usage:
  Example: 	1) perl data_extract_by_ids.pl -idfile id.txt -destfile dest.txt -out out.txt
  		2) perl data_extract_by_ids.pl -idfile id.txt -destfile dest.txt -out out.txt -idcol 2
  		3) perl data_extract_by_ids.pl -idfile id.txt -destfile dest.txt -out out.txt -destcol 3
  		4) perl data_extract_by_ids.pl -idfile id.txt -destfile dest.txt -out out.txt -idcol 2 -destcol 3
  		5) perl data_extract_by_ids.pl -idfile id.txt -destfile dest.txt -out out.txt -idcol 2 -filter
  Options:
  forced parameters:
