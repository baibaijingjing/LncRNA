...
#	foreach my $range(keys %{$hash{$type}})
#	{
#		if($hash{$type}{$range}>5)
#		{
#			print OUT "$type\t$range\t$hash{$type}{$range}\n";
#		}
#	}
#}
...
my $dir = dirname($out);
`Rscript $Bin/R/barplot.r --infile $out --outfile $dir/circRNA.length.distribution --x.col 3 --y.col 1 --y.lab "Length Count(log2)" --x.lab "Length Range" --skip 1 --title.lab "CircRNA Length Distribution"`;
#######################################################################################
