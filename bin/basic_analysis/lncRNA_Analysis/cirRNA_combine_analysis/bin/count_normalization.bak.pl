...
&USAGE unless ($fIn and $fOut and $dir and $totalRead);
...
pop @head;
my @norm = @head;
s/(T\d+)$/$1_norm_count/ foreach @norm;
my $norm = join("\t",@norm);
my $head = join("\t",@head);
print OUT "$circRNA\t$head\t$norm\n";

my %reads;
open(R,$totalRead) or die $!;
while(<R>)
{
	my ($s,$reads_count) = split(/\t+/,$_);
	$reads{$s}=$reads_count;
}
close R;
while  (<IN>) {
		...
		for(my $i=0;$i<@head;$i++){
			my $sample = $head[$i];
			my $norm_count = $line[$i]*1000000/$reads{$sample};
...
Program Date:   2012.07.02
 ...
 Description:   This program is used to ......
       Usage:
		...
		-dir <file>   reads count file ,forced
        -h      help
