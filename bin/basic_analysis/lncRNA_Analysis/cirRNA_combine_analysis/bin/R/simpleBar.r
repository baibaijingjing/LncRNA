...
# create df
data[,opt$x.col] <- paste("chr",data[,opt$x.col])
df <- aggregate(data[,opt$y.col], by=list(y=data[,opt$x.col]), FUN=sum)
#df<- df[1:10,]
df<- df[df$x>100,]
df <- data[df$y==data[,opt$x.col],]
df <- data.frame(x=as.factor(df[,opt$x.col]), y=df[,opt$y.col])
