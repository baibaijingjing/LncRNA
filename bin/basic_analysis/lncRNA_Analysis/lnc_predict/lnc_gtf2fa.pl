...
############################################
my %opts;
GetOptions(\%opts,"fa=s","gtf=s","gff=s","o=s","h");
if (!defined($opts{fa})||!defined($opts{gtf})||(!defined($opts{o}))||!defined($opts{gff})) {
	&help();
}
...
#`perl $Bin/../predict_target/bin/gtf_to_gff.pl -i $gtf -o $out/merged.gff`;
open OUT, ">$out";
...
#	my ($id) = (split /\s+/, $head)[1];
	...
	$locus=~s/-//g;
	if (exists $ha{$id}){
        	print OUT ">$id $ha{$id}{'chr'} $ha{$id}{'star'}-$ha{$id}{'end'}\n$fasta{$head}\n";
	}
	#print OUT  ">$id\t$locus\t$se\n$fasta{$head}\n";
}
#`rm  $temp `;
...
sub help{
	...
	Usage:
		...
		-gff	<STR>	GFF FILE			must be given;
