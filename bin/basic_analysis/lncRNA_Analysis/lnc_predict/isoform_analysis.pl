...
#my %hash;
close OUT2;
`$config{Rscript} $Bin/oneFactorBox.r --infile $od/plot.txt --outfile $od/lnc_vs_mRNA.isoform.png --value.col 2 --x.col 1 --x.lab type --y.lab isoform --title.lab lnc_vs_mRNA`;
sub get_geneID{
	my ($file,$hash)=@_;
	my %hash;
	open(IN,$file )or die "$!";
