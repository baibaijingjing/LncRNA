...
#my $CPAT_BIN = "cpat.pl";
#my $CPC_BIN     = "$Bin/software/cpc-0.9-r2/bin/run_predict_local_v2.sh";
...
if ($step==4){
	...
	print OUT "#seq_id\thmm_acc\thmm_name\thmm_start\thmm_end\thmm_length\tbit_score\tE-value\n";
	while(<IN>){
		next if /^#/ || /^$/;
		my @pfm=split /\s+/,$_;
		my $id=(split /./,$pfm[0])[0];
		#print OUT "$id\t".join("\t",@pfm[5,6,8,9,10,11,12])."\n";
		print OUT join("\t",@pfm[0,5,6,8,9,10,11,12])."\n";
	}
	close OUT;
	close IN;
