...
# bin
#==================================================================
my $RNAHybrid_BIN = $config{RNAhybrid};
my $miranda_BIN = $config{miranda};
...
$para{miRanda} = 1 unless (defined $para{miRanda});
## if the user forget setting parameter for miRanda, using the following default values
$para{miRanda_strict} = 0 unless (defined $para{miRanda_strict});
$para{miRanda_SCORE} = 140.0 unless (defined $para{miRanda_SCORE}) ;
$para{miRanda_EN} = -25 unless (defined $para{miRanda_EN}) ;
...
#######targetfinder parameter
$para{TargetFinder_score} = 3 unless (defined $para{TargetFinder_score});
$para{TargetFinder_rev} = 0 unless (defined $para{TargetFinder_rev});
$para{RNAhybrid_EN} = "-25" unless (defined $para{RNAhybrid_EN});
my %seq;
...
if ($step==0) {
	...
	if ($type==0) {
		...
		&Cut_shell_qsub("$work_sh/$key.TargetFinder.sh","$para{CPU}","$para{Memory}","$para{Queue_type}");
		$step=1;
		#}
...
sub Cut_shell_qsub {#Cut shell for qsub 1000 line one file
        ...
        my $vf = shift;
         ...
         if ($line<=1000) {
			...
			close IN;
			close OUT;
            #&cmd_call( "sh /share/nas2/genome/bmksoft/tool/qsub_sge_plus/v1.0/qsub_sge.plus.sh $shell --reqsub -maxproc 20 --independent");
            &cmd_call( "sh $config{qsub_sh} --reqsub -maxproc $cpu --independent --queue $queue --resource vf=$vf $shell.div.sh");
         }
         if ($line>=1000) {
                   ...
                   foreach my $div_file (@div) {
						# &cmd_call( "sh /share/nas2/genome/bmksoft/tool/qsub_sge_plus/v1.0/qsub_sge.plus.sh $div_file --reqsub -maxproc 20 --independent");
						 &cmd_call( "sh $config{qsub_sh} --reqsub -maxproc $cpu --independent --queue $queue --resource vf=$vf  $div_file");
