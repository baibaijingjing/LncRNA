...
&USAGE unless ($mi_lnc and $lnc_m  and $in1 and $od);
&MKDIR($od);
$od=&ABSOLUTE_DIR($od);
	...
	if (defined $mi_m and $mi_m ne ""){
		foreach my $sg (keys %m_s){
			...
			foreach my $s (@mirna){
				...
				if (exists $degm{sg} && exists $degs{$s}) {print OUT1 "$s\t$sg\tsm\n";}
			}
		}
	}	
...
----------------------------------------------------------------------------------------------------
   ...
   Example:
            perl $Script -mi_m ./*.mir2target.list -lnc_m ./novel_lncRNA_target.xls  -mi_lnc /lncRNA_target2mirna.list -in1 Lnc_Diff_Analysis  -od  ./
----------------------------------------------------------------------------------------------------
USAGE
        print $usage;
        exit;
