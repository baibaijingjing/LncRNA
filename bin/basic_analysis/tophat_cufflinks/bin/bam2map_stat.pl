...
open IN, "$config{samtools} view $bam|" or die;
 #bedtools bamtobed -i 
while (<IN>) {
    chomp;
    ...
    if (64 & $flag) {
        ...
        $left_uniq_mapped_reads{$qid} = 1 if ($mapQ ==50);
    } else {
        $right_mapped_reads{$qid} = 1;
        $right_uniq_mapped_reads{$qid} = 1 if ($mapQ ==50);
    }
...
my %map_strand;
open IN, " $config{bedtools} bamtobed -i $bam|" or die;
 #bedtools bamtobed -i 
while (<IN>) {
    chomp;
