...
#########################
my $python = "/share/nas2/genome/biosoft/Python/2.7.2/bin/python"; # 2014-12-18 ~ 
#my $gene_model_to_splicegraph = "/share/nas2/genome/biosoft/SpliceGrapher/current/gene_model_to_splicegraph.py";
my $gene_model_to_splicegraph = "/share/nas2/genome/biosoft/SpliceGrapher/0.2.4/gene_model_to_splicegraph.py"; # 2014-12-18 ~ 
...
&Cut_shell_qsub ("$ARGV[1]/work_sh/$ARGV[2].SpliceGraphgff.sh",30,"1G","general.q");
...
sub Cut_shell_qsub {#Cut shell for qsub 1000 line one file
	...
	if ($line<=1000) {
		if ($notename=~/login\-0\-4/) {
			system "qsub-sge.pl $shell --queue $queue --maxproc $cpu --resource vf=$vf --reqsub";
		}
		else
		{
			system "ssh login-0-4 qsub-sge.pl $shell --queue $queue --maxproc $cpu --resource vf=$vf --reqsub ";
		}
	}
	if ($line>1000) {
		...
		foreach my $div_file (@div) {
			if ($notename=~/login\-0\-4/) {
				system "qsub-sge.pl $div_file --queue $queue --maxproc $cpu --resource vf=$vf --reqsub";
			}
			else
			{
				system "ssh login-0-4 qsub-sge.pl $div_file --queue $queue --maxproc $cpu --resource vf=$vf --reqsub ";
			}
		}
	}
}
