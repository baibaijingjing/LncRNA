...
usage=function(){
  cat("
      ...
      example:
		  Rscript plot_insertsize.R   infile=T05.insertSize.r.list outfile=inset_size.png bg=T
		  Rscript plot_insertsize.R   infile=T05.insertSize.r.list outfile=inset_size.png bg=F
		  \n"
  )
  q(status=1)
}
...
gplot=function(infile,outfile="plot.png",sep="",skip=0,geom1="geom_point",geom2=NULL,
  ...
  mytheme$axis.title.x$size=11*size_ratio
  mytheme$axis.title.y$size=11*size_ratio
  ...
  p=ggplot(data,aes(x,y))+theme_classic()+
    do.call(geom1,plotpar)+ylim(0,max(data$y))
...
x=1,y=2,width=120,height=80,
xlim=NULL,ylim=NULL,xlab="Inset Size(bp)",ylab="Number of Reads",title=NULL,bg=T)
