...
while (<IN>) {
    ...
    if ( $tmp[0] =~ m/Sample/ ) {
        ...
        if ( !-f "$od/totalRead.stat.xls" ) {
            my $total_line = `less -S $sample{$tmp[1]}{FQ1} | wc -l `;
            chomp $total_line;
...
if ( $step == 1 ) {
    ...
    if ( !-f "$idx_prefix.1.bt2" ) {
        ...
        elsif ( -e "$genome_path/$idx_prefix.1.bt2l" )
        {    # for large genomes more than about 4 billion nucleotides in length
...
if ( $step == 2 ) {
    print STDOUT
      "=== Align the RNA-seq read to genome using Tophat2 + bowtie2  ===\n";
    ...
    foreach my $sam ( sort keys %sample ) {
        ...
        print OUT
"$TOPHAT_BIN --output-dir ./ --read-mismatches $para{Mismatch} --read-edit-dist $para{Mismatch} --max-intron-length 5000000 --library-type $para{Lib_type} --num-threads 6 --GTF $od/Ref_Genome/$gtf --mate-inner-dist $para{Insert_size} $od/Ref_Genome/$idx_prefix $sample{$sam}{FQ1} $sample{$sam}{FQ2} && ";
    ...
    $para{CPU} ||= "30";
    &Cut_shell_qsub( "$od/work_sh/Tophat.sh","$para{CPU}" , "$para{Memory}","$para{Queue_type}" );
...
if ( $step == 3 ) {
    print STDOUT "=== Scripture-Cufflinks Assembly Analysis  ===\n";
    print STDOUT "shell file: $od/work_sh/Scripture-Cufflinks.sh\n";
    print STDOUT "shell file: $od/work_sh/bed2gtf.sh\n";
    print $log "=== Scripture-Cufflinks Assembly Analysis  ===\n";
    print $log "shell file: $od/work_sh/Scripture-Cufflinks.sh\n";
    print $log "shell file: $od/work_sh/bed2gtf.sh\n";
    open OUT,  ">$od/work_sh/Scripture-Cufflinks.sh" || die;
    open OUT2, ">$od/work_sh/bed2gtf.sh"             || die;
    my %beds;
    &MKDIR("$od/Scripture");
    &MKDIR("$od/Cufflinks");
    ...
    foreach my $sam ( sort keys %sample ) {
        ...
        print OUT
"$CUFFLINKS_BIN -o ./ -p 4 -g $od/Ref_Genome/$gtf --library-type $para{Lib_type} -u -L $sam $od/Tophat/$sam/accepted_hits.bam \n";
        &MKDIR("$od/Scripture/$sam");
        foreach my $chromo (@chromosome) {
            print OUT "cd $od/Scripture/$sam && ";
            print OUT
"$config{Blast2G0java} -Xmx10g -jar $SCRIPTURE_BIN  -alignment  $od/Tophat/$sam/accepted_hits.bam  -out $chromo.scriptureESTest.segments.bed  -sizeFile $genome_size -chr $chromo   -chrSequence $od/Ref_Genome/$chromo.fa \n";
            print $chromo;
            push @{ $beds{$sam} }, "$chromo.scriptureESTest.segments.bed";
        }
        print OUT2 "cd $od/Scripture/$sam && cat @{$beds{$sam}} >$od/Scripture/$sam.bed && ";
        print OUT2 "cd $od/Scripture && perl $Bin/bin/bed2gtf.pl -gtf $sam.gtf  -bed $sam.bed \n";
    }
    close OUT;
    close OUT2;
    $para{Memory}     ||= "30G";
    $para{Queue_type} ||= "general.q";
    $para{CPU} ||= "30";
    &Cut_shell_qsub( "$od/work_sh/Scripture-Cufflinks.sh","$para{CPU}", "$para{Memory}", "$para{Queue_type}" );
    &Check_qsub_error("$od/work_sh/Scripture-Cufflinks.sh");
    &Cut_shell_qsub( "$od/work_sh/bed2gtf.sh", "$para{CPU}", "$para{Memory}", "$para{Queue_type}" );
    &Check_qsub_error("$od/work_sh/bed2gtf.sh");

    $step++ unless ($oneStepOnly);
    print STDOUT "Scripture-Cufflinks Finished \n";
    print $log "Scripture-Cufflinks Finished\n";
}
...
if ( $step == 4 ) {
    print STDOUT
      "=== Cuffcompare: compare gtf of sample with known gene model ===\n";
    print STDOUT "cuffcompare shell file: $od/work_sh/Cuffcompare.sh\n";
    print $log
      "=== Cuffcompare: compare gtf of each sample with known gene model ===\n";
    ...
    foreach my $sam ( sort keys %sample ) {
        ...
        &MKDIR("$od/Compare");
        print OUT1
"$CUFFCOMPARE_BIN -r $od/Ref_Genome/$gtf $od/Cufflinks/$sam/transcripts.gtf \n";
        print OUT2 "$od/Cufflinks/$sam/transcripts.gtf\n";
        print OUT2 "$od/Scripture/$sam.gtf\n";
    }
    print OUT3 "cd $od/Compare/ && ";
    print OUT3
"$CUFFCOMPARE_BIN -i $od/assembly_GTF_list.txt -o first  -r $od/Ref_Genome/$gtf  && ";
...
if ( $step == 5 ) {
    print STDOUT
      "=== Cuffquant and Cuffnorm analysis(only use gene FPKM result) ===\n";
    ...
    print $log
      "=== Cuffquant and Cuffnormanalysis(only use gene FPKM result) ===\n";
    ...
    #cut gtf
    #
    system "perl $Bin/bin/comparegtf_cut.pl -gtf $od/Compare/Compare.gtf -od $od/Compare/ -cut 2000 ";
    #
    # write shell
    #
    ...
    foreach my $di ( glob "$od/Compare/CUTGTF/cut*gtf" ) {
        my $dii = basename($di);
        my $name = ( split /.gtf/, $dii )[0];
        &MKDIR("$od/Cuffquant/$name");
        &MKDIR("$od/Cuffnorm/$name");

        print OUT2
"$CUFFNORM_BIN --library-type $para{Lib_type} -o $od/Cuffnorm/$name -p 6 -total-hits-norm -no-update-check -output-format cuffdiff $di ";
        print OUT2 "-L ";
        my $lable;
        my $sam_str;
        foreach my $sam ( sort keys %sample ) {
            &MKDIR("$od/Cuffquant/$name/$sam");
            print OUT1 "cd $od/Cuffquant/$name/$sam && ";
            print OUT1
"$CUFFQUANT_BIN -o ./ -p 6 -b $od/Ref_Genome/$genome --library-type $para{Lib_type}  $di $od/Tophat/$sam/accepted_hits.bam \n";
            $lable   .= "$sam" . ",";
            $sam_str .= "$od/Cuffquant/$name/$sam/abundances.cxb ";
        }
        $lable =~ s/,$//;
        $sam_str =~ s/\s+$//;
    ...
    $para{Queue_type} ||= "general.q";
    $para{CPU} ||= "30";
    &Cut_shell_qsub( "$od/work_sh/Cuffquant.sh", "$para{CPU}", "$para{Memory}", "$para{Queue_type}" );
    &Check_qsub_error("$od/work_sh/Cuffquant.sh");
    &Cut_shell_qsub( "$od/work_sh/Cuffnorm.sh", "$para{CPU}", "$para{Memory}", "$para{Queue_type}" );
    &Check_qsub_error("$od/work_sh/Cuffnorm.sh");
    #########################
    #cat gene_count_tracking and isoforms.fpkm_tracking
    #################
	`cat $od/Cuffnorm/cut*/genes.count_tracking > $od/Cuffnorm/genes.count_tracking `;
	`cat $od/Cuffnorm/cut*/genes.fpkm_tracking > $od/Cuffnorm/genes.fpkm_tracking `;
	`cat $od/Cuffnorm/cut*/isoforms.fpkm_tracking > $od/Cuffnorm/isoforms.fpkm_tracking`;
	`cat $od/Cuffnorm/cut*/isoforms.count_tracking > $od/Cuffnorm/isoforms.count_tracking `;
	`cat $od/Cuffnorm/cut*/isoforms.read_group_tracking > $od/Cuffnorm/isoforms.read_group_tracking `;
	`cat $od/Cuffnorm/cut*/genes.read_group_tracking > $od/Cuffnorm/genes.read_group_tracking `;
=pod
	  my @cout=glob("$od/Cuffnorm/cut*/genes.count_tracking");
		my $cout_join=join (" ",@cout);
		#`cat $cout_join > $od/Cuffnorm/genes.count_tracking `;
		`cat $od/Cuffnorm/cut*/genes.count_tracking > $od/Cuffnorm/genes.count_tracking `;
		my @gfpkm=glob("$od/Cuffnorm/cut*/genes.fpkm_tracking");
		my $gfpkm_join=join (" ",@gfpkm);
		`cat $gfpkm_join > $od/Cuffnorm/genes.fpkm_tracking `;
		my @fpkm=glob("$od/Cuffnorm/cut*/isoforms.fpkm_tracking");
		my $fpkm_join=join (" ",@fpkm);
		`cat $fpkm_join > $od/Cuffnorm/isoforms.fpkm_tracking`;
		my @icou=glob("$od/Cuffnorm/cut*/isoforms.count_tracking");
		my $icon_join=join (" ",@icou);
		`cat $icon_join > $od/Cuffnorm/isoforms.count_tracking `;
		my @read=glob("$od/Cuffnorm/cut*/isoforms.read_group_tracking");
		my $read_join=join(" ",@read);
		`cat $read_join > $od/Cuffnorm/isoforms.read_group_tracking `;
		my @gread=glob("$od/Cuffnorm/cut*/genes.read_group_tracking");
		my $gread_join=join(" ",@gread);
		`cat $gread_join > $od/Cuffnorm/genes.read_group_tracking `;
    #$step++ unless ($oneStepOnly);
=cut
...
Usage:
	...
	-s                step of the program   option,default 1;
                            ...
                            3  Scripture and Cufflinks Assembly Analysis
