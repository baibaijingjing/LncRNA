...
foreach my $chr (sort keys %track2chr) {
	foreach my $gene_track (sort keys %{$track2chr{$chr}}) {
		print OUT1 "$chr\tCufflinks\tgene\t$gene_boundary{$gene_track}{Start}\t$gene_boundary{$gene_track}{End}\t.\t$gene_boundary{$gene_track}{Strand}\t.\tID=$new_gene_track2name{$gene_track}\n";
		foreach my $iso_track (sort keys %{$new_gene_iso{$gene_track}}) {
			print OUT1 "$chr\tCufflinks\tmRNA\t$iso_info{$gene_track}{$iso_track}{Start}\t$iso_info{$gene_track}{$iso_track}{End}\t.\t$iso_info{$gene_track}{$iso_track}{Strand}\t.\tID=$new_iso_track2name{$iso_track}".";Parent=$new_gene_track2name{$gene_track}\n";
			my @exons = sort {$a<=>$b} @{$iso_info{$gene_track}{$iso_track}{Exonarray}};
			my $length;
			for (my $i=0 ; $i < $#exons ; $i+=2) {
				print OUT1 "$chr\tCufflinks\tCDS\t$exons[$i]\t$exons[$i+1]\t.\t$iso_info{$gene_track}{$iso_track}{Strand}\t.\tParent=$new_iso_track2name{$iso_track}\n";
