...
my $fpkm_file="$cuffdiff_dir/genes.fpkm_tracking";
my $count_file="$cuffdiff_dir/genes.count_tracking";
my @sample;
...
print OUT "#gene_id\tlength\tstrand\tlocus\t$fpkm_counts\n";

for my $i (sort {$track{$a}{'gene_id'} cmp $track{$b}{'gene_id'}} keys %track) {
	my $basic_inf=join "\t",($track{$i}{'gene_id'},$track{$i}{'length'},$track{$i}{'stand'},$track{$i}{'locus'});
	my $print=$basic_inf;
...
for (my $s=0;$s<@sample;$s++) {
	...
	open (EXP,">$outdir/$sample[$s].geneExpression2.xls") or die $!;
	print EXP "#gene_id\tlength\tstrand\tlocus\t$sample[$s]\_FPKM\t$sample[$s]\_count\n";
...
Program Date:	2014.04.15
      Modify:	2015.09.15 <linhj\@biomarker.com.cn> 
