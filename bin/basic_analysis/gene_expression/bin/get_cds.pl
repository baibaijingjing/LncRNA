...
open OUT,">$ARGV[2].gene2transcript.list" || die $!;
...
open OUT2,">$ARGV[2].longest_transcript.fa" || die $!;
open OUT3,">$ARGV[2].gene2longest_transcript.list" || die $!;
...
foreach my $chr (sort keys %gene2trans) {
	foreach my $gene (sort keys %{$gene2trans{$chr}}) {
		...
		foreach my $iso (sort keys %{$gene2trans{$chr}{$gene}}) {
			...
			if (!defined $trans_max_len) {
				$trans_max_len = $trans_len;
				$trans_max_id = $iso;
				$trans_max_seq = $trans_seq;
			}
			elsif ($trans_len > $trans_max_len) {
				$trans_max_len = $trans_len;
				$trans_max_id = $iso;
				$trans_max_seq = $trans_seq;
			}
		}
		print OUT2 ">$gene\t$trans_max_id\n$trans_max_seq\n";
		print OUT3 "$gene\t$trans_max_id\n";
	}
}
