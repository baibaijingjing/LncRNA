...
# GetOptions
# ------------------------------------------------------------------
my ( $detail_cfg, $data_cfg,$analysis_dir, $step, $data_analyzer, $customer_service_exe ,$oneStepOnly,$odir);
...
if ( $step == 1 ) {
	...
	$cmd .="perl $Bin/bin/Result_extract.pl --id $analysis_dir --od $web_report_dir -cfg $data_cfg -cfg2 $detail_cfg ";
...
######################### xml report
if ( $step == 2 ) {
    $cmd = "perl $Bin/bin/LncRNA_xml4.pl -indir $web_report_dir -config $detail_cfg -xml $web_report_dir/configtest_long.xml  ";
...
if ( $step == 4 ) {
    ...
    $cmd="$config{python} $Bin/bin/htmlConvert/xml2HtmlConverter.py -i $web_report_dir/configtest_xmlconvert.xml -o $HTML";
...
----------------------------------------------------------------------------------------------------
   ...
   Example:
            perl $Script --cfg data.cfg  --cfg2 detail.cfg --dir Analysis/
