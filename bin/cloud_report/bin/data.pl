...
while (<IN>) {
	chomp;
	next if (/^#/);
	my ($sam_ID,$BMK_ID,$readsum,$basesum,$GC,$N,$Q30) = split /\t/,$_;
...
########### Get $new_gene_num
chomp (my $new_gene_num = `grep ">" $dir_abs/BMK_3_mRNA/BMK_1_NewGene/*.newGene.longest_transcript.fa |wc -l`)	;
...
close IN;
if (!defined $new_gene_ann_num ) {
    $new_gene_ann_num = "0";
}

########## Get $deg_num, $lnc_num, $diff_lnc, $optimized_gene_num
chomp (my $deg_num = `less $dir_abs/BMK_3_mRNA/BMK_*_DEG_Analysis/BMK_1_All_DEG/All.DEG.Expression.xls |wc -l`);
$deg_num = $deg_num-1;

chomp (my $diff_lnc = `less $dir_abs/BMK_2_LncRNA/BMK_*_DEG_Analysis/BMK_1_All_DEG/All.DEG.Expression.xls |wc -l`);
#$diff_lnc = $diff_lnc-1;
if ($diff_lnc ==0) {
    $diff_lnc =0;
}else{
...
chomp (my $lnc_num = `less $dir_abs/BMK_2_LncRNA/BMK_2_LncRNA_Prediction/LncRNA.id |wc -l`);
$lnc_num=$lnc_num-1;

my $optimized_gene_path = glob("$dir_abs/BMK_3_mRNA/BMK_*_Gene_Structure_Optimize/*.geneStructure.optimize.xls");
...
sub get_table {
	chomp (my $id = shift);
	%table_info=(
		"样品测序数据评估统计表" => "$dir_abs/BMK_1_rawData/BMK_1_Data_Assess/AllSample_GC_Q.stat",
		"Clean Data与参考基因组比对结果统计表" => "$dir_abs/BMK_1_rawData/BMK_2_Mapped_Statistics/*mappedStat.xls",
		"新基因功能注释结果统计" => "$dir_abs/BMK_3_mRNA/BMK_*_NewGene/BMK_*_NewGene_Anno/Function_Annotation.stat.xls",
	);
