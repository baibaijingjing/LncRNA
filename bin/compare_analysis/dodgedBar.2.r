...
xaxistext<-factor(as.character(data[,opt$x.col]),levels = unique(as.character(data[,opt$x.col])))
df <- data.frame(x=as.numeric(xaxistext),x.lab=xaxistext, group=as.factor(data[,opt$group.col]), y=log2(data[,opt$y.col]+1))

#df <- data.frame(x=as.numeric(as.factor(data[,opt$x.col])),x.lab=as.factor(data[,opt$x.col]), group=as.factor(data[,opt$group.col]), y=log2(data[,opt$y.col]))
