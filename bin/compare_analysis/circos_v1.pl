...
this script set many default set value in circos config and make useres very easy to
create a circos  graphics in short time;

B<contact:huangls>

=head1 SYNOPSIS

perl circos_v1.pl [options] [file ...]

=item B<example> 

perl circos_v1.pl --chr demo.chr --circle R3_R2.Target.gene.scaffold.fst_scatter --type scatter --od test --outfile demo.chr --circle R3_R2.Target.gene.scaffold.fst_scatter --type line
...
=item B<--chr> <character>  

input chr ideogram file , required

=item B<--outfile> <character>  

output file name,default of "circos"

=item B<--od>  <character>  

outpath,default get_cwd;optional

=item B<--circle> <character>	

input files of each Tracks;file format must correspond to the type required;

=item B<--type> <character>	

the type of  plot data Tracks to draw, corresponding to --circle; 
suppoerted [line,scatter,heatmap,histogram,highlight,text,link];


        	
=item B<--chromosomes_units>	<character>	

default	1000000 ;chromosome scale to show ticks;if your chr contig or scaffold  relative shorter
,then set this value smaller;  

=item B<--chr_spacing>	<character>	

default	'10u';set the gap size between chr;

=item B<--chr_radius>	<character>	

default	'0.90r';set global radius��of circos to draw;if your chr name is very long,set this option smaller will be useful;

=item B<--chr_thickness>	<character>	

default	'20p';set chr width;

=item B<--chr_fill>	<character>	

default	'yes';wether give the chr a color;

=item B<--chr_stroke_color>	<character>	

default	'dgrey'

=item B<--chr_stroke_thickness>	<character>	

default	'2p'

=item B<--chr_show_label>	<character>	

default	'yes'

=item B<--chr_label_radius>	<character>	

default	'60p'

=item B<--chr_label_size>	<character>	

default	'30'

=item B<--chr_label_parallel>	<character>	

default	'yes'

=item B<--gap_thickness>	<character>	

default	'0.015'

=item B<--thick> <character> 

default '0.1' for each track,

=item B<--max_circle_out_pisition>	<character>	

default	'0.99'

=item B<--show_ticks>	<character>	

default	'yes'

=item B<--show_tick_labels>	<character>	

default	'yes'

=item B<--tick_label_size>	<character>	

default	'20p'

=item B<--tick_label_offset>	<character>	

default	'10p'

=item B<--tick_format>	<character>	
...
use newPerlBase;
my %config1=%{selectconf("/share/nas1/niulg/pipline/v2.2.5/Config/")};
...
my($chromosomes_units,$chr_spacing,$chr_radius,$chr_thickness,$chr_fill,$chr_stroke_color,$chr_stroke_thickness,$chr_show_label,
	$chr_label_radius,$chr_label_size,$chr_label_parallel,$gap_thickness,$max_circle_out_pisition,$circle_width)=(1000000,'10u','0.90r','20p','yes','dgrey','2p','yes','60p','30','yes','0.01','0.99','0.1');

#------------ticks of chr initial var-------------------------------------------#
my($show_ticks,$show_tick_labels,$tick_label_size,$tick_label_offset,$tick_format)=('yes','yes','20p','10p','%d');
...
print  "$config1{circos} -conf $od/$outfile.conf -outputdir $od -outputfile $outfile\n";
system ("$config1{circos} -conf $od/$outfile.conf -outputdir $od -outputfile $outfile");
#system ("/share/nas2/genome/biosoft/circos/current/bin/circos -conf $od/$outfile.conf -outputdir $od -outputfile $outfile");
#print  "/home/hygine/下载/circos-0.69-2/bin/circos -conf $od/$outfile.conf -outputdir $od -outputfile $outfile\n";
#system ("/home/hygine/下载/circos-0.69-2/bin/circos -conf $od/$outfile.conf -outputdir $od -outputfile $outfile");
}
