...
# create df
df <- data.frame(x=as.factor(data[,opt$x.col]), y=data[,opt$y.col])
...
p <- p + geom_text(aes(label=round(y,2)),hjust=0.5, vjust=-0.5,size=4 ) 
}
# remove legend
p <- p + theme(legend.position = "none")

# set order
p <- p + scale_x_discrete(limits=as.factor(data[,opt$x.col])) 
# grid and background
