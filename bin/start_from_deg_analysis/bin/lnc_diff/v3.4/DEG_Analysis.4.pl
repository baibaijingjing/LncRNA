...
&show_log2("step_5: Difference expression genes analysis of lncRNA start.");
###########################################################
...
foreach my $dir (glob "$od/*_vs_*") {
	...
	my $cmd="perl $Bin/bin/filter_by_names/v1.0/filter_by_names.l.pl -i $dir/$group_name.final.tmp.xls -lnc $lnclist -o $dir/$group_name.final.xls && ";	
	$cmd.="perl $Bin/bin/filter_by_value/v1.0/filter_by_value.pl -i $dir/$group_name.final.xls -o $dir/$group_name.DEG_final.xls  ";
...
#	print SH2 "perl $Bin/bin/draw_cor_plot/v1.0/draw_cor_plot.pl -treated $treated -control $control -all $dir/$group_name.final.xls -o $dir/$group_name.cor.png \n";
	...
	if (defined $enrich) {
		...
		###########得到差异lncRNA对应的靶基因列表
		#print SH3 "perl $Bin/bin/Enrichment/get_diff_lnctarget_gene.pl -i $dir/$group_name.DEG_final.xls -t $od/../Basic_Analysis/Tophat_Cufflinks/Lnc_target_predict/novel_lncRNA_target.xls -o $dir/Anno_enrichment/$group_name.target.list \n";
		print "perl $Bin/bin/Enrichment/get_lnc_target_gene.pl -l $dir/$group_name.DEG_final.xls -tg $lnctar -out $dir/Anno_enrichment/$group_name.lncRNA_DEG_targene.xls -m $id2/$group_name/$group_name.final.xls  \n\n ";
		print SH1 "perl $Bin/bin/Enrichment/get_lnc_target_gene.pl -l $dir/$group_name.DEG_final.xls -tg $lnctar -out $dir/Anno_enrichment -k $group_name -m $id2/$group_name/$group_name.final.xls  \n\n";
		print SH3 "perl $Bin/bin/Enrichment/select_DEG_Anno.pl -i $enrich -deg $dir/Anno_enrichment/$group_name.lncRNA_DEG_targene.xls -o $dir/Anno_enrichment/$group_name.annotation.xls\n";
		print SH3 "perl $Bin/bin/Enrichment/draw_DEG_GO_graph.pl -i $enrich -deg $dir/Anno_enrichment/$group_name.lncRNA_DEG_targene.xls -k $group_name -od $dir/Anno_enrichment\n";
		print SH3 "perl $Bin/bin/Enrichment/draw_top_GO_graph.pl -i $enrich -deg $dir/Anno_enrichment/$group_name.lncRNA_DEG_targene.xls -k $group_name -od $dir/Anno_enrichment/\n";
		print SH3 "perl $Bin/bin/Enrichment/draw_DEG_KEGG_graph.pl -i $enrich -deg $dir/Anno_enrichment/$group_name.lncRNA_DEG_targene.xls -k $group_name -od $dir/Anno_enrichment\n";
		print SH3 "perl $Bin/bin/Enrichment/draw_DEG_COG_graph.pl -i $enrich -deg $dir/Anno_enrichment/$group_name.lncRNA_DEG_targene.xls -k $group_name -od $dir/Anno_enrichment\n\n";
		#print SH3 "perl $Bin/bin/Enrichment/draw_DEG_KOG_graph.pl -i $enrich -deg $dir/Anno_enrichment/target_gene.list -k $group_name -od $dir/Anno_enrichment\n\n";
	}
}
