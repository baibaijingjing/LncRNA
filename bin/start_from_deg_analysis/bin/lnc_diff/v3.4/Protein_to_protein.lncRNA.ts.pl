...
# GetOptions
# ------------------------------------------------------------------
my ($flinks,$unigene,$ftest,$od,$protein,$blast,$id,$clade_or_taxid,$lnc);
...
&USAGE unless ( $unigene and $od and $id and $lnc);
# ------------------------------------------------------------------
# 
# ------------------------------------------------------------------
#my $STRING_db = "/share/nas2/nas/nas1/database/STRING/v10"; 
my $STRING_db = "/share/nas4/yangxh/database/STRING/v10";
...
if (-d $id ) {
	` cat $id/*_vs_*/Anno_enrichment/*DEG*targene.xls |grep -v "#" |cut -f 1|sort |uniq >$od/used_gene.list `;
}
elsif (-f $id) {
...
$cmd="perl $Bin/util/blast_process.pl -od $od -fa $od/used_gene.list.fa  -e 1e-5 -p blastx -no $protein_seq ";
...
foreach my $dir (glob "$id/*_vs_*") {
	...
	my %deg=();
	my %degl=();
	my @genes=glob("$dir/Anno_enrichment/$group_name*DEG*targene.xls");
	my $lncid="$dir/$group_name.DEG_final.xls";
	open (OUT,">$dir/$group_name.lnc_mRNA.CytoscapeInput.txt")||open (OUT2,">$od/$group_name.lnc_mRNA.CytoscapeInput.txt");
	open (ATT,">$dir/$group_name.lnc_mRNA.Cyto_Attribute.txt")||open (OUT2,">$od/$group_name.lnc_mRNA.Cyto_Attribute.txt");
	my $len=@genes;
	my $le=@lncid;
	next if ($len==0);
	...
	my %test;	
	my $nam=basename$id;
	open (OUT2,">$od/$nam.CytoscapeInput.txt")||die $!;
