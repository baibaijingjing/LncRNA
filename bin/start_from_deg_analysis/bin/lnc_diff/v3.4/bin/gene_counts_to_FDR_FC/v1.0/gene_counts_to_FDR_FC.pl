...
sub control_DE_analysis{
	...
	if( exists $$control{Sep} ) {
		...
		my $len = @$sep_groups;
		for(my $i=0; $i<$len; $i++) {
			compare_with_replicates($$sep_groups[$i], $outdir, $in);
		}
	}
	close SH;
	&Shell_qsub ("$sh_dir/gene_counts_to_FC_FDR.sh",$para{'Queue_type'},$para{'CPU'});
}
...
#			my $cmd = " cd $Bin/bin/ && /share/nas2/genome/biosoft/R/3.1.1/lib64/R/bin/Rscript $PROGRAM{de_with_ebseq} $read_count_file ";
			#my $cmd = "cd $Bin/bin/ && /share/nas2/genome/biosoft/R/3.1.1/lib64/R/bin/Rscript $PROGRAM{de_with_ebseq} $read_count_file ";
			my $cmd = "cd $Bin/bin/ && $config{Rscript} ebseq_analysis.r $read_count_file ";
...
#			my $cmd = " cd $Bin/bin/ && /share/nas2/genome/biosoft/R/3.1.1/lib64/R/bin/Rscript $PROGRAM{de_with_deseq} $read_count_file ";
			#my $cmd = "cd $Bin/bin/ && /share/nas2/genome/biosoft/R/3.1.1/lib64/R/bin/Rscript $PROGRAM{de_with_deseq} $read_count_file ";
			my $cmd = "cd $Bin/bin/ && $config{Rscript} deseq_analysis.r $read_count_file ";
