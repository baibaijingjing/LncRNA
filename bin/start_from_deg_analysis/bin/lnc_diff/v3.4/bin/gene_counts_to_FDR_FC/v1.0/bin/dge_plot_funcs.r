...
exp_lab="log2(FPKM)", FC_lab="log2(FC)", FDR_lab="-log10(FDR)", ma_main="MA plot", vo_main="Volcano plot") {
	# check null
...
### NOTE: here using FPKM stand for expression
#################################################################
plot_corr <- function(file=NULL, fpkm1=NULL, fpkm2=NULL, xlab="log10(FPKM1)", ylab="log10(FPKM2)", main="correlation plot") {
	# check args
	# check null
	...
	# plot
	p <- qplot(log10(fpkm1), log10(fpkm2), xlab=xlab, ylab=ylab, main=main, size=I(0.5))
