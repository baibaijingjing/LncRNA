...
$opts{map} ||= "/share/nas3/xugl/Database/KEGG/png/latest/";
$opts{func} ||= "both";
...
unless ($opts{func} eq "kegg") 
{
	...
	system "perl $Bin/gene_ontology_graph_v1.2.pl -i $opts{i}/$go_list -i $opts{k}.GO.list.txt -mark All_Unigene -mark DEG_Unigene -o $go_out -k $opts{k}" ;                               #modified by sunhy  change to new script
