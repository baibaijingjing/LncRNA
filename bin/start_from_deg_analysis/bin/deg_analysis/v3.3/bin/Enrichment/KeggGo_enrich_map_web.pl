...
#	��GO ���и�������
#	��kegg �Ƚ��и�������������ͨ·ͼ
#	keggͨ·ͼǶ����ҳ����javascript�ű���ʾ��Ϣ
#	
#
#	��ע��
#	koͼ��������
#	�� kegg����飬Ϊ�˺����ĸ�������?
#	��term�жϣ�û�в���������������ͼѡ�õĶ����в�������ko ����һ�μ��٣�
#	��ͼʱ��ͼ��û��ko��������ͼ  ���ڶ��μ��٣�
#
#������������������������������������������������
...
&help() if ( defined($opts{h}) || !defined($opts{i}) ||
			...
			!defined($opts{d}) || !defined($opts{map}) );
...
my %file_db;
&file_cfg_read($opts{map},\%file_db);
...
unless ($opts{func} eq "kegg") 
{
	...
	system "perl $Bin/gene_ontology_graph_v1.2.pl -i $opts{i}/$go_list -i $opts{k}.GO.list.txt -mark All_Unigene -mark DEG_Unigene -o $go_out -k $opts{k} -c $file_db{component_file} -f $file_db{function_file} -p $file_db{process_file} " ;                               #modified by sunhy  change to new script
...
#        print OUT $header;
	...
	$\ = "\n";
	{
		...
		while (<IN>) {
				...
				next if $m <2;                                         ##reduce pathway which deg number is smaller than2;
				my $p_value = &hyper($m,$M,$n,$N);
				my $m_ratio = "$m out of $n ".100 * $m / $n.'%';
				my $M_ratio = "$M out of $N ".100 * $M / $N.'%';
				$hash{$term}{name} = $term;
...
sub help {#usage
	...
	Usage:
		...
		-func            [option]
		      ...
		      both       [default]
		-map  <file>     file db config 
 		-v               [option]   view process on screen
	
	Example��

		perl KeggGo_enrich_map_web.pl -d xxx.DEG.final.xls -k demo -i input -o output -func kegg  -map /share/nas2/genome/pipeline_cfg/db_file.cfg
