...
unless ($key eq "kegg"){
    $anno=(glob "$fIn/GO_tree.stat_File_*")[0];
...
unless ($key eq "go"){
    $anno = (glob "$fIn/Kegg.pathway_File_*")[0];
...
open IN,"$deg/All.DEG_final.xls" or die $!;
while (<IN>){
    chomp;
    next if (/^#/ || /^$/);
    my $gene=(split /\s+/,$_)[0];
...
open DE,"$deg/All.DEG_final.xls" or die $!;
my $head_line=<DE>;
...
foreach my $final (sort{$sort{$b}<=>$sort{$a}} keys %sort){
    ...
    my $line= "$final($sort{$final})";
    
    my $line1="$final($sort{$final})";
    my $line2="$final($sort{$final})";
...
system "$config{Rscript} $Bin/anno_cluster_heatmap.r --infile $list_file --outfile $od/$pre --rowname T --height 2000 --width 2000 --color \"blue,white,red\" --size 4 ";
#$kegg{num}{match}=keys %{$kegg{match}};
#$kegg{num}{all}=keys %{$kegg{all}};
