...
for( i in 1:(length(sam_name)-1) ){
	for( j in (i+1):length(sam_name) ){
		...
		p <- p + theme_bw()
		png(filename=paste(args[2],"/cor_plot/",s1,"_vs_",s2,".cor.png", sep=""), height = 750, width = 750, res = 125, units = "px")
		print(p)
		dev.off()
	}
}
...
# output
file <- paste(args[2], "/sample_cluster.png", sep="")
...
file <- paste(args[2], "/free_com.cor", sep="")
