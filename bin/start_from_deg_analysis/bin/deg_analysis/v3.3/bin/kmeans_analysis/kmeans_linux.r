...
par(mfrow=c(opt$rownum,opt$colnum),mar=c(1.2, 1.3, 1.2, 0.4) + 0.1,mgp=c(0.0,0.05,0),tck=0.02, cex.axis=0.6,cex.lab=0.5,
...
for (i in 1:opt$cluster) {
  ...
  matplot(t(part),type='l',lty=1,col=c(rep(cut[i],len),rep("black",3)),
          axes=F,main=paste("Cluster",i,sep=""),xlab="",ylab="",ylim=range(data))
