...
# calculate FPKM
sam_name <- colnames(count_data)#[ !(colnames(count_data)%in% colnames(count_data))]#c("geneLength")) ]
...
#	keep <- c > 0
#r <- 10^9 * c[keep] / sum(c[keep]) / count_data[keep]#[,"geneLength"][keep]
	log10fpkm=log10(c+1)
...
# create data.frame
log10fpkm <- data.frame(log10fpkm=all, sample="i")
